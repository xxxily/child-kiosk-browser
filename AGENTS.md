...
- WebView debugging is process-global: all production changes must go through `WebViewDebuggingGate`. Experimental CDP continuity must remain default-off, restricted to an explicitly approved trusted Origin, and bounded by a short lease with socket-close verification; never silently enable persistent Chrome Inspect.
