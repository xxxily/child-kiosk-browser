...
- **历史列表进一步简化**：移除站点首字母徽标和独立的打开按钮，点击整条记录即可打开网页。
- **操作图标改为透明样式**：加入白名单和删除仅保留图标，保留原有触控范围；“清空浏览历史”移到“最近浏览”卡片右侧并仅显示图标。

### Tests - 测试

- Standard / Enhanced 单元测试、Debug 与 Release APK 构建通过。
- Standard / Enhanced WebView task manifest 验证通过。

## [0.4.29] - 2026-08-09

### Changed - 变更

- **历史列表改为紧凑行式布局**：标题、站点和访问时间集中在一行信息层级，打开、加入白名单和删除改为小图标操作，减少列表高度并提升浏览效率。
- **悬浮球入口顺序调整**：浏览分组中的“历史”移到最后，常用的主页、收藏、前进后退和刷新操作保持连续。
...
- **悬浮球浏览历史入口**：首页和网页内的悬浮球浏览分组新增“历史”按钮，共用独立的浏览历史 Activity；从记录恢复访问时通过 Activity Result 返回原宿主，避免锁定模式重复创建 WebView Activity 或破坏现有标签状态。
- **预设应用网站图标自动缓存**：默认白名单应用在真正打开后才延迟发现网站图标，成功后写入独立 `site_icon_path` 并落盘缓存；主页、白名单管理和应用选择器优先使用本地图标，失败或文件缺失时回退原有内置图标。

### Changed - 变更

- **浏览历史界面重排**：历史页从 `AdminConsoleScreen.kt` 抽成可复用组件，改用 LazyColumn 按日期分组；摘要、操作按钮和记录卡片会按窄屏手机、横屏短高设备与 600dp+ 平板自适应排列，避免长中文、URL 和底部动作裁切。
- **图标读取与并发策略收敛**：白名单列表不再为预设应用批量联网抓图；自动发现采用单通道、同应用合并，下载按 URL 合并并限制并发，同时补充 SVG 解码与异步本地图片加载。

### Fixed - 修复

- **浏览历史跨宿主重复写入**：将 30 秒同 URL 防重从单个 Activity 内存变量下沉到 Room 事务，近时间重复回调会更新同一行并清理多余记录；数据库 v9 → v10 升级时同步合并已有重复数据，超过窗口的真实再次访问仍保留。
- **网站图标失败回退**：缓存文件不存在、图片解码失败或网络读取失败时，不再显示破图或通用占位，直接使用该应用原有的自定义内置图标。

### Tests - 测试

- 新增历史记录事务去重、已有重复行收敛、真实重复访问保留、网站图标缓存优先级及预设应用自动发现范围单测。
- Standard / Enhanced 双发行版 Kotlin 编译、单元测试与 Debug APK 构建通过。
...
- 新增浏览历史去重与预设网站图标缓存运行手册，记录跨 Activity/进程根因、事务边界、缓存策略、诊断信号和回归步骤。
...
- **修复悬浮窗保活无法授权的问题**：v0.4.19 的悬浮窗保活缺少 `SYSTEM_ALERT_WINDOW` 权限声明（该声明在 v0.4.14 回退 overlay 时被移除、v0.4.19 恢复时遗漏），导致应用不出现在系统"显示在其他应用上层"列表中、`canDrawOverlays()` 恒为 false、Overlay Keep-Alive 从未生效。已补回权限声明，安装后即可在系统设置中为本应用授权悬浮窗权限。

### 建议验证

1. 更新安装后，进入 设置 → 应用 → 特殊应用权限 → 显示在其他应用上层 → 儿童空间 → 允许。
2. 可信站点切后台/息屏 5 分钟以上：诊断出现 `overlay_attached` 且无 `freeze` 事件，心跳持续 `RESPONSIVE`。
3. 亮屏解锁后无白屏、无强制重载，输入法正常。
...
- **解冻机制升级**：实测确认单次 `WebView.onResume()` 无法解冻 Android 16 / WebView 150 的隐藏页面冻结（v0.4.17 期间解冻后页面从未恢复心跳，仅前台恢复才生效）。解冻尝试升级为组合拳：`onResume()` + 视图可见性属性短暂切换（`INVISIBLE→恢复`，仅触发 Chromium 重新评估自身可见性，不触碰窗口与输入法）+ `onResume()`。
- **解冻有效性验证**：每次解冻 10 秒后校验主线程心跳是否恢复，输出 `webview_unfreeze_ineffective` 事件（含连续失败次数），日志可直接判断解冻手段是否生效。
- **无效降频**：连续 3 次解冻无效后，解冻尝试降频至每 5 分钟一次，避免无效循环刷屏与无意义操作。

### 建议验证

1. 可信站点切后台 5 分钟以上，观察诊断是否出现 `webview_unfrozen` 后心跳恢复（组合拳生效），或 `webview_unfreeze_ineffective`（平台冻结无法后台解冻）。
2. 息屏 5 分钟以上，确认 `screen_off` 后页面行为。
3. 若日志持续显示 `webview_unfreeze_ineffective`，说明该平台冻结只能等待前台恢复，需要评估物理可见窗口方案（悬浮窗/PiP）或调整产品预期。
...
- **紧急回退 v0.4.15 的后台可见性保持（Background Continuity）机制**：该机制在后台/息屏时向 Chromium 伪造 `VISIBLE`/`SCREEN_STATE_ON`，实测导致窗口级输入通道与 Chromium 输入状态失步，**所有网页（可信与非可信）输入法均无法唤起**，与 v0.4.0–v0.4.9 的历史回归完全一致，且未能解决后台定时器节流问题。
- 恢复 v0.4.14 行为：托管 WebView 重新使用原生 `WebView`，移除 `PersistentWebView`、`PAUSED` 生命周期上报与相关 UI/测试；网页输入框恢复原生输入连接与系统输入法。
- 结论记录：Chromium 输入连接是窗口级状态，伪造 View/窗口可见性在任何时机都不安全；物理悬浮窗方案（v0.4.13）存在 Surface 剥离缺陷，亦非已验证路径。后台"前台级"持续运行受平台限制，当前仅保证 FGS/WakeLock/渲染器优先级下的进程与资源存活。

## [0.4.15] - 2026-08-01

### Added - 新增

- 可信高性能页面新增后台持续运行（Background Continuity）机制：托管标签页改用受控的 `PersistentWebView`，仅当页面处于活动高性能会话**且**宿主 Activity 已暂停/停止（真实后台或息屏）时，向 Chromium 上报 `VISIBLE`/`SCREEN_STATE_ON`，阻止 Blink 对页面触发定时器节流（Background Throttling）与任务挂起（Task Suspend），页面定时器与网络请求在后台/息屏下保持前台级执行。
- Activity 生命周期新增 `PAUSED` 状态上报，用于精确判定页面是否进入后台保活窗口。

### Fixed - 修复

- 修复仅依赖 FGS/WakeLock/渲染器优先级时，后台与息屏场景网页定时器和网络请求仍被 Chromium 内核节流或冻结的问题。
- 输入法安全边界：可见性保持仅在后台态启用，前台（STARTED/RESUMED）时所有回调保持 100% 原生，避免 v0.4.10 修复过的网页输入框 IME 问题回归；WebView 不再移出 Activity 布局树，避免 v0.4.13 的 Surface 销毁/白屏问题复现。

### 建议验证

1. 开启“高性能持续运行”并将站点加入可信名单，确认会话 ACTIVE、FGS/WakeLock 就绪。
2. 切换 App 至后台 5 分钟以上，确认网页定时器与网络请求持续执行（可观察诊断心跳与站点业务日志）。
3. 息屏 5 分钟以上，确认页面持续运行；亮屏恢复前台后页面无白屏、无强制重载。
4. 对可信页面与普通页面分别验证文本/密码/数字/多行输入框可正常调起系统输入法，前台→后台→前台循环后输入仍正常。
