...
> 创建日期：2026-07-10  
> 更新日期：2026-08-04  
> 状态：v0.4.26 阶段实现与双设备生产验证完成，进入长时与跨版本持续验证  
...
> 2026-08-04 阶段实现、Android 13/16 实测矩阵、失败路线和后续架构建议统一见
> [Android WebView 后台 / 息屏连续性阶段总结](../background_continuity_phase_summary_2026-08-04.md)。
...
截至 v0.4.26，当前实现已经收敛为“可信页面存活、hidden 低频连续性和前台恢复增强”：
Android 13 Xiaomi 与 Android 16 OnePlus 的正式包样本都能保持同一 PID、session 和 load ID，
但 hidden 稳态主线程约 60 秒一次、诊断 Dedicated Worker 约 5 秒一次。该证据不能解释为页面
获得了前台等价执行能力。完整结论与证据见
[阶段总结](../background_continuity_phase_summary_2026-08-04.md)。
...
完成这些工作后，指定站点在灭屏与后台状态下的页面存活与恢复可靠性会明显高于普通 WebView 页面；但系统回收、OEM 冻结、WebView renderer 退出和 Chromium 隐藏页调度仍可能造成中断。需求、UI、验收和发布说明必须始终保留这一边界。

产品必须明确遵守以下承诺边界：

- 高性能模式提供的是页面存活、低频调度证据和恢复增强，不提供前台等价执行承诺。
- 实验性 CDP 续行默认关闭，只能用于明确批准的可信 Origin、短租约和可审计诊断场景。
- 秒级定位、可靠 WebSocket、轮询、上传、计时或不可丢失事件必须迁移到 Android 原生服务或服务端，并通过持久化队列、幂等协议和恢复同步与 WebView 协作。
